...
describe('SequelizeAccessContextResolver', () => {
  ...
  it('selects the first active membership when no preferredOrgId', async () => {
    const user = User.fromPersistence({
      id: 'u1', email: 'u@t.com', username: 'U1CODE1', identification: null, fullName: null, avatarFileId: null, status: 'active',
  ...
  it('uses preferredOrgId when user is active member', async () => {
    const user = User.fromPersistence({
      id: 'u2', email: 'u2@t.com', username: 'U2CODE2', identification: null, fullName: null, avatarFileId: null, status: 'active',
  ...
  it('ignores preferredOrgId when membership is inactive', async () => {
    const user = User.fromPersistence({
      id: 'u3', email: 'u3@t.com', username: 'U3CODE3', identification: null, fullName: null, avatarFileId: null, status: 'active',
      isPlatformAdmin: false, permissionsVersion: 0,
      createdAt: new Date(), updatedAt: new Date(),
    });
    await users.save(user);
  ...
  it('returns pv even without org', async () => {
    const user = User.fromPersistence({
      id: 'u-pv', email: 'u@t.com', username: 'UPVCODE', identification: null, fullName: null, avatarFileId: null, status: 'active',
